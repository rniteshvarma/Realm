...
`,Pe={electric:Q,ocean:Ne,lava:Me,ice:Ae,circuit:ke};function De({orbRadius:e,phase:s,isMobile:o}){const i=o?2:4,r=a.useRef();a.useRef([]);const t=a.useMemo(()=>Array.from({length:i},(m,d)=>({radius:.04+Math.random()*.04,orbitR:e*1.6+d*.3,speed:.4+Math.random()*.6,axisX:Math.random()*.6-.3,axisY:Math.random()*.3,phaseOff:d/i*Math.PI*2})),[e,i]),l=a.useMemo(()=>new G(.06,8,8),[]),c=a.useMemo(()=>new le({color:13421789,roughness:.8,metalness:.2}),[]),u=a.useMemo(()=>t.map(m=>{const d=[];for(let b=0;b<=60;b++){const x=b/60*Math.PI*2+m.phaseOff,w=Math.cos(x)*m.orbitR,v=Math.sin(x)*m.orbitR,g=Math.sin(x*m.axisX+m.phaseOff)*m.orbitR*.2;d.push(new C(w,g,v))}return new q().setFromPoints(d)}),[t]),p=a.useMemo(()=>t.map(()=>new H({color:8952251,transparent:!0,opacity:0})),[t]),h=a.useMemo(()=>new ce,[]);return N(m=>{const d=m.clock.elapsedTime;if(!r.current)return;const b=s==="orbit",x=s==="approach";t.forEach((w,v)=>{const g=d*w.speed+w.phaseOff,f=Math.cos(g)*w.orbitR,y=Math.sin(g)*w.orbitR,R=Math.sin(g*w.axisX+w.phaseOff)*w.orbitR*.2;if(h.position.set(f,R,y),h.scale.setScalar(w.radius/.06),h.updateMatrix(),r.current.setMatrixAt(v,h.matrix),p[v]){const k=x||b?.35:0;p[v].opacity+=(k-p[v].opacity)*.03}}),r.current.instanceMatrix.needsUpdate=!0}),n.jsxs("group",{children:[n.jsx("instancedMesh",{ref:r,args:[l,c,i]}),u.map((m,d)=>n.jsx("line",{geometry:m,material:p[d]},d))]})}function Ie({project:e,phase:s,phaseProgress:o,mouseNDC:i,isActive:r,isMobile:t}){const l=a.useRef(),c=a.useRef();a.useRef();const u=a.useRef(1),p=Pe[e.orbShader]||Q,h=a.useMemo(()=>({uTime:{value:0},uSpeed:{value:.4},uColor1:{value:e.orbColor1.clone()},uColor2:{value:e.orbColor2.clone()}}),[e]),m=a.useMemo(()=>new V({vertexShader:Ee,fragmentShader:p,uniforms:h}),[p,h]),d=t?32:64,b=a.useMemo(()=>new G(e.orbRadius,d,d),[e.orbRadius,d]);N(v=>{const g=v.clock.elapsedTime;h.uTime.value=g;const f=s==="approach"?.4+o*.8:s==="orbit"?1.2:.4;if(h.uSpeed.value+=(f-h.uSpeed.value)*.02,!l.current)return;const y=!r&&(s==="descent"||s==="inside")?0:1;if(u.current+=(y-u.current)*.04,l.current.scale.setScalar(u.current),c.current&&s==="orbit"){const R=i.x*.4,k=-i.y*.25;c.current.rotation.y+=(R-c.current.rotation.y)*.05,c.current.rotation.x+=(k-c.current.rotation.x)*.05}c.current&&(c.current.rotation.y+=.001)});const x=a.useMemo(()=>new se(e.orbRadius*1.02,e.orbRadius*1.18,64),[e.orbRadius]),w=a.useMemo(()=>new ie({color:e.orbColor2,transparent:!0,opacity:.08,side:U,blending:W,depthWrite:!1}),[e.orbColor2]);return n.jsxs("group",{ref:l,position:e.orbPosition,children:[n.jsx("mesh",{ref:c,geometry:b,material:m}),n.jsx("mesh",{geometry:x,material:w,rotation:[Math.PI/2,0,0]}),r&&n.jsx(De,{orbRadius:e.orbRadius,phase:s,isMobile:t}),s==="approach"&&o>.6||s==="orbit"?n.jsx(ye,{position:[0,e.orbRadius+.4,0],fontSize:.18,color:"#C9A84C",anchorX:"center",anchorY:"bottom",fillOpacity:s==="orbit"?1:o*2-1.2,children:e.title}):null]})}const K=`// ── INTERIOR BIOME: NEURAL GRID ──
...
`,Be={neuralGrid:K,liquidMetal:_e,digitalGarden:Oe,frequencyRoom:Fe,archiveWithin:Le},je=`
varying vec2 vUv;
void main() {
  vUv = uv;
  gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
}
`;function ze({interiorType:e,visible:s}){const{viewport:o}=$(),i=a.useRef(),r=a.useMemo(()=>({uTime:{value:0},uResolution:{value:new Y(window.innerWidth,window.innerHeight)}}),[]),t=Be[e]||K;return N(l=>{i.current&&(r.uTime.value=l.clock.elapsedTime)}),s?n.jsxs("mesh",{position:[0,0,-3],renderOrder:-1,children:[n.jsx("planeGeometry",{args:[o.width*2.5,o.height*2.5,1,1]}),n.jsx("shaderMaterial",{ref:i,vertexShader:je,fragmentShader:t,uniforms:r,depthWrite:!1,depthTest:!1})]}):null}function Ve({position:e,rotation:s,hovered:o}){const i=a.useMemo(()=>new ve(new fe(1.82,1.12)),[]),r=a.useMemo(()=>new H({color:"#C9A84C",transparent:!0,opacity:.3}),[]);return N(()=>{r&&(r.opacity=o?.8:.3)}),n.jsx("lineSegments",{position:e,rotation:s,geometry:i,material:r})}function Ge({position:e,rotation:s,color:o,label:i,visible:r}){const t=a.useRef(),[l,c]=a.useState(!1),u=a.useRef(),p=a.useMemo(()=>new C(e[0],e[1],e[2]+1.2),[e]),h=a.useMemo(()=>new C(...e),[e]),m=a.useMemo(()=>new C(1.4,1.4,1.4),[]),d=a.useMemo(()=>new C(1,1,1),[]);N(v=>{if(!t.current||!r)return;const g=v.clock.elapsedTime;l?(t.current.position.lerp(p,.08),t.current.rotation.y+=(0-t.current.rotation.y)*.1,t.current.scale.lerp(m,.08)):(t.current.position.lerp(h,.06),t.current.scale.lerp(d,.06)),u.current&&(u.current.uniforms.uTime.value=g)});const b=a.useMemo(()=>({uTime:{value:0},uColor:{value:new M(o)}}),[o]),x=`
    varying vec2 vUv;
    void main() {
      vUv = uv;
      gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
  ...
  `;return r?n.jsxs("group",{children:[n.jsxs("mesh",{ref:t,position:e,rotation:s,onPointerOver:()=>c(!0),onPointerOut:()=>c(!1),children:[n.jsx("planeGeometry",{args:[1.8,1.1,1,1]}),n.jsx("shaderMaterial",{ref:u,vertexShader:x,fragmentShader:w,uniforms:b,transparent:!0,roughness:.1,metalness:.3})]}),n.jsx(Ve,{position:e,rotation:s,hovered:l})]}):null}function z({position:e,label:s,visible:o,onClick:i}){const r=a.useRef(),t=a.useRef(),[l,c]=a.useState(!1),u=a.useRef(0),p=a.useMemo(()=>new ue(.18,0),[]);return N(h=>{if(!r.current||!o)return;const m=h.clock.elapsedTime;r.current.rotation.y+=.012,r.current.rotation.x=Math.sin(m*.5)*.15;const d=l?1:0;if(u.current+=(d-u.current)*.08,r.current.material&&(r.current.material.emissiveIntensity=u.current*1.5),t.current){const b=l?1+Math.sin(m*4)*.06:0;t.current.scale.setScalar(b+(l?1:0)),t.current.material.opacity=l?.6:0}}),o?n.jsxs("group",{position:e,children:[n.jsx("mesh",{ref:r,geometry:p,onPointerOver:()=>c(!0),onPointerOut:()=>c(!1),onClick:i,children:n.jsx("meshStandardMaterial",{color:"#C9A84C",emissive:"#C9A84C",emissiveIntensity:0,metalness:.9,roughness:.1})}),n.jsxs("mesh",{ref:t,rotation:[Math.PI/2,0,0],children:[n.jsx("ringGeometry",{args:[.22,.28,32]}),n.jsx("meshBasicMaterial",{color:"#C9A84C",transparent:!0,opacity:0,side:U,depthWrite:!1})]})]}):null}function We({project:e,phase:s,phaseProgress:o,onReadMore:i}){const r=a.useRef(),t=s==="inside"||s==="escape",l=a.useMemo(()=>[{position:[-2,.5,-1.5],rotation:[0,.15,.05],color:_(e.orbColor2)},{position:[1.8,.2,-2.5],rotation:[0,-.2,-.03],color:_(e.orbColor1)},{position:[-1.2,-.8,-3.5],rotation:[.05,.1,0],color:_(e.orbColor2)},{position:[.5,1.1,-2],rotation:[-.05,-.15,.06],color:_(e.orbColor1)}],[e]),c=t&&o>.92;return N(()=>{r.current&&(r.current.visible=t)}),n.jsxs("group",{ref:r,visible:t,children:[n.jsx(ze,{interiorType:e.interiorType,visible:t}),l.map((u,p)=>n.jsx(Ge,{position:u.position,rotation:u.rotation,color:u.color,label:`${e.title} — IMG ${p+1}`,visible:t&&o>.05},p)),n.jsx(z,{position:[-.6,-1.2,-1.8],label:"VIEW LIVE",visible:c,onClick:()=>window.open(e.liveUrl,"_blank")}),n.jsx(z,{position:[.6,-1.2,-1.8],label:"READ MORE",visible:c,onClick:i}),n.jsx("pointLight",{position:[0,1,-1],intensity:1.5,color:e.orbColor2,distance:8}),n.jsx("ambientLight",{intensity:.3})]})}function _(e){return`#${e.clone().multiplyScalar(.5).getHexString()}`}const P=["THE BRIEF","THE PROCESS","THE SOLUTION","THE IMPACT","THE LEARNING"],Ue=["brief","process","solution","impact","learning"];function qe({project:e,onClose:s}){const o=a.useRef(),i=a.useRef(),[r,t]=a.useState(0),[l,c]=a.useState(!1),u=a.useRef(null);a.useEffect(()=>{o.current&&O.fromTo(o.current,{y:"100%",opacity:0},{y:"0%",opacity:1,duration:.6,ease:"power3.out"})},[]);const p=a.useCallback(()=>{l||(c(!0),O.to(o.current,{y:"100%",opacity:0,duration:.5,ease:"power3.in",onComplete:s}))},[l,s]),h=a.useCallback(()=>{r<P.length-1&&t(f=>f+1)},[r]),m=a.useCallback(()=>{r>0&&t(f=>f-1)},[r]);a.useEffect(()=>{const f=y=>{y.key==="ArrowRight"&&h(),y.key==="ArrowLeft"&&m(),y.key==="Escape"&&p()};return window.addEventListener("keydown",f),()=>window.removeEventListener("keydown",f)},[h,m,p]);const d=f=>{u.current=f.touches[0].clientX},b=f=>{if(u.current===null)return;const y=f.changedTouches[0].clientX-u.current;y<-50&&h(),y>50&&m(),u.current=null};a.useEffect(()=>{i.current&&O.fromTo(i.current,{x:30,opacity:0},{x:0,opacity:1,duration:.4,ease:"power2.out"})},[r]);const x=(e==null?void 0:e.panels)||{},w=Ue[r],v=P[r],g=x[w]||"[PLACEHOLDER: Fill case study content]";return n.jsxs("div",{ref:o,className:"project-overlay",onTouchStart:d,onTouchEnd:b,style:{transform:"translateY(100%)"},children:[n.jsx("div",{className:"project-overlay__glass"}),n.jsx("button",{className:"project-overlay__close",onClick:p,"aria-label":"Close case study",children:n.jsxs("svg",{viewBox:"0 0 24 24",fill:"none",xmlns:"http://www.w3.org/2000/svg",children:[n.jsx("line",{x1:"4",y1:"4",x2:"20",y2:"20",stroke:"currentColor",strokeWidth:"1.5",strokeLinecap:"round"}),n.jsx("line",{x1:"20",y1:"4",x2:"4",y2:"20",stroke:"currentColor",strokeWidth:"1.5",strokeLinecap:"round"})]})}),n.jsxs("div",{className:"project-overlay__inner",children:[n.jsxs("div",{className:"project-overlay__left",children:[n.jsxs("div",{className:"overlay-project-number",children:["0",((e==null?void 0:e.index)??0)+1]}),n.jsx("h2",{className:"overlay-project-title",children:e==null?void 0:e.title}),n.jsx("p",{className:"overlay-project-role",children:e==null?void 0:e.role}),n.jsx("p",{className:"overlay-project-year",children:e==null?void 0:e.year}),n.jsx("div",{className:"overlay-project-tags",children:((e==null?void 0:e.tags)||[]).map(f=>n.jsx("span",{className:"overlay-tag",children:f},f))}),n.jsx("div",{className:"overlay-dots",children:P.map((f,y)=>n.jsx("button",{className:`overlay-dot ${y===r?"active":""}`,onClick:()=>t(y),"aria-label":`Go to ${P[y]}`},y))})]}),n.jsxs("div",{className:"project-overlay__right",children:[n.jsxs("div",{ref:i,className:"overlay-panel",children:[n.jsxs("div",{className:"overlay-panel__header",children:[n.jsxs("span",{className:"overlay-panel__number",children:["0",r+1," / 0",P.length]}),n.jsx("h3",{className:"overlay-panel__title",children:v})]}),n.jsx("p",{className:"overlay-panel__body",children:g}),w==="impact"&&(e==null?void 0:e.metric)&&n.jsxs("div",{className:"overlay-metric",children:[n.jsx("span",{className:"overlay-metric__number",children:e.metric}),n.jsx("span",{className:"overlay-metric__desc",children:e.metricDesc})]})]}),n.jsxs("div",{className:"overlay-nav",children:[n.jsx("button",{className:`overlay-nav__btn overlay-nav__btn--prev ${r===0?"disabled":""}`,onClick:m,disabled:r===0,"aria-label":"Previous panel",children:"←"}),n.jsx("span",{className:"overlay-nav__label",children:v}),n.jsx("button",{className:`overlay-nav__btn overlay-nav__btn--next ${r===P.length-1?"disabled":""}`,onClick:h,disabled:r===P.length-1,"aria-label":"Next panel",children:"→"})]})]})]})]})}const He=`// ── STAR FIELD VERTEX SHADER ──
...
`;function Ye({warpStrength:e,isMobile:s}){const o=s?1e3:3e3,i=a.useRef(),{positions:r,sizes:t,brightnesses:l,seeds:c}=a.useMemo(()=>{const h=new Float32Array(o*3),m=new Float32Array(o),d=new Float32Array(o),b=new Float32Array(o);for(let x=0;x<o;x++)h[x*3]=(Math.random()-.5)*300,h[x*3+1]=(Math.random()-.5)*300,h[x*3+2]=-30-Math.random()*200,m[x]=.5+Math.random()*2.5,d[x]=.3+Math.random()*.7,b[x]=Math.random();return{positions:h,sizes:m,brightnesses:d,seeds:b}},[o]),u=a.useMemo(()=>({uTime:{value:0},uWarpStrength:{value:0},uWarpSeed:{value:0}}),[]),p=a.useMemo(()=>new V({vertexShader:He,fragmentShader:$e,uniforms:u,transparent:!0,blending:W,depthWrite:!1,vertexColors:!1}),[u]);return N(h=>{u.uTime.value=h.clock.elapsedTime,u.uWarpStrength.value+=(e-u.uWarpStrength.value)*.04}),n.jsx("points",{ref:i,material:p,children:n.jsxs("bufferGeometry",{children:[n.jsx("bufferAttribute",{attach:"attributes-position",count:o,array:r,itemSize:3}),n.jsx("bufferAttribute",{attach:"attributes-aSize",count:o,array:t,itemSize:1}),n.jsx("bufferAttribute",{attach:"attributes-aBrightness",count:o,array:l,itemSize:1}),n.jsx("bufferAttribute",{attach:"attributes-uWarpSeed",count:o,array:c,itemSize:1})]})})}function Xe({visible:e,position:s}){const o=a.useRef();return N(i=>{if(!o.current||!e)return;const r=i.clock.elapsedTime;o.current.rotation.y+=.008,o.current.rotation.x=Math.sin(r*.4)*.06}),e?n.jsxs("mesh",{ref:o,position:s,children:[n.jsx("coneGeometry",{args:[.08,.25,8]}),n.jsx("meshStandardMaterial",{color:"#C9A84C",emissive:"#C9A84C",emissiveIntensity:.6,metalness:.9,roughness:.1})]}):null}function Qe({path:e,visible:s,opacity:o}){const i=a.useRef(),r=a.useMemo(()=>{const l=[];for(let c=0;c<=50;c++)l.push(e.getPoint(c/50));return l},[e]),t=a.useMemo(()=>new q().setFromPoints(r),[r]);return N(()=>{i.current&&(i.current.opacity+=(s?o:0-i.current.opacity)*.03)}),n.jsx("line",{geometry:t,children:n.jsx("lineBasicMaterial",{ref:i,color:"#C9A84C",transparent:!0,opacity:0,linewidth:1})})}function Ke({scrollProgress:e,currentProjectIndex:s,currentPhase:o}){const{camera:i}=$(),r=a.useRef(new C(0,0,-10)),t=a.useRef([]);return a.useEffect(()=>{const l=[];T.forEach((c,u)=>{const p=u>0?T[u-1].orbPosition:null;l.push(X(c,p))}),t.current=l},[]),N(()=>{if(t.current.length===0)return;const l=T.length,c=1/l,u=Math.min(Math.floor(e/c),l-1),p=(e-u*c)/c,h=t.current[u];if(!h)return;const m=h.getPoint(Math.min(p,.99));i.position.lerp(m,.08);const d=T[u];r.current.lerp(d.orbPosition,.05),i.lookAt(r.current)}),null}function Ze({phase:e,phaseProgress:s}){a.useRef({x:0,y:0});const o=a.useMemo(()=>new Y(0,0),[]);return N(()=>{let i=0;e==="descent"&&(s<.5?i=s*2*.018:i=(1-s)*2*.018);const r=o.x,t=r+(i-r)*.15;o.set(t,t*.3)}),n.jsx(pe,{children:n.jsx(ge,{blendFunction:be.NORMAL,offset:o})})}function Je({project:e,visible:s,phaseProgress:o}){const i=s&&o>.1;return n.jsxs("div",{className:`orbit-meta ${i?"orbit-meta--visible":""}`,children:[n.jsxs("div",{className:"orbit-meta__number",style:{transitionDelay:"0ms"},children:["0",e.index+1]}),n.jsx("h2",{className:"orbit-meta__title",style:{transitionDelay:"80ms"},children:e.title}),n.jsx("p",{className:"orbit-meta__role",style:{transitionDelay:"160ms"},children:e.role}),n.jsx("p",{className:"orbit-meta__year",style:{transitionDelay:"240ms"},children:e.year}),n.jsx("div",{className:"orbit-meta__tags",style:{transitionDelay:"320ms"},children:e.tags.map(r=>n.jsx("span",{className:"orbit-tag",children:r},r))}),n.jsx("p",{className:"orbit-meta__pull-quote",style:{transitionDelay:"400ms"},children:e.pullQuote})]})}function en({project:e,phase:s,phaseProgress:o}){const i=s==="inside",r=i&&o>.12,t=i&&o>.38,l=i&&o>.56,c=i&&o>.76,u=a.useMemo(()=>{if(!(e!=null&&e.metric))return"";const m=e.metric.replace(/[^\d.]/g,""),d=e.metric.replace(/[\d.]/g,"");return{digits:parseFloat(m),suffix:d}},[e]),[p,h]=a.useState(0);return a.useEffect(()=>{if(!l||!u.digits)return;const m=performance.now(),d=1500,b=x=>{const w=Math.min((x-m)/d,1),v=1-Math.pow(1-w,3);h(Math.round(v*u.digits)),w<1&&requestAnimationFrame(b)};requestAnimationFrame(b)},[l,u]),n.jsxs("div",{className:"narrative-blocks",children:[n.jsx("div",{className:`narrative-block narrative-block--a ${r?"visible":""}`,children:e==null?void 0:e.verb}),n.jsx("div",{className:`narrative-block narrative-block--b ${t?"visible":""}`,children:e==null?void 0:e.challenge}),n.jsxs("div",{className:`narrative-block narrative-block--c ${l?"visible":""}`,children:[n.jsxs("span",{className:"narrative-metric",children:[p,u.suffix]}),n.jsx("span",{className:"narrative-metric-desc",children:e==null?void 0:e.metricDesc})]}),n.jsxs("div",{className:`narrative-block narrative-block--d ${c?"visible":""}`,children:['"',e==null?void 0:e.reflection,'"']})]})}function nn({visible:e,onComplete:s}){const o=a.useRef(),i=a.useRef();return a.useEffect(()=>{if(!e||!o.current)return;const t=Array.from(o.current.querySelectorAll(".entry-title__letter"));O.timeline({onComplete:s}).fromTo(t,{opacity:0,scale:.4,filter:"blur(12px)"},{opacity:1,scale:1,filter:"blur(0px)",duration:.06,stagger:{from:"center",amount:.8},ease:"power2.out"}).fromTo(i.current,{opacity:0,y:20},{opacity:1,y:0,duration:.8,ease:"power2.out"},"-=0.3").to([o.current,i.current],{delay:1.2,opacity:0,filter:"blur(8px)",y:-20,scale:1.05,duration:.6,stagger:.1,ease:"power3.in"})},[e,s]),e?n.jsxs("div",{className:"entry-title-container",children:[n.jsx("h1",{ref:o,className:"entry-title",children:"III. THE CARTOGRAPHY".split("").map((t,l)=>n.jsx("span",{className:"entry-title__letter",children:t===" "?" ":t},l))}),n.jsx("p",{ref:i,className:"entry-subtitle",children:"WORLDS I HAVE BUILT."})]}):null}function tn({scrollProgress:e,projectIndex:s,phase:o,totalProjects:i}){const r=e,t=o==="inside"||o==="descent",l=o==="escape"&&s<i-1,c=T[s+1];return n.jsxs(n.Fragment,{children:[n.jsxs("div",{className:`cartography-realm-label ${t?"dimmed":""}`,children:[n.jsx("div",{className:"realm-label__title",children:"III. THE CARTOGRAPHY"}),n.jsxs("div",{className:"realm-label__world",children:["WORLD ",s+1," / ",i]})]}),n.jsxs("div",{className:"cartography-progress-bar",children:[n.jsx("span",{className:"progress-label progress-label--top",children:"START"}),n.jsx("div",{className:"progress-track",children:n.jsx("div",{className:"progress-dot",style:{top:`${r*100}%`}})}),n.jsx("span",{className:"progress-label progress-label--bottom",children:"END"})]}),n.jsxs("div",{className:`scroll-hint-container ${["orbit","escape"].includes(o)?"visible":""}`,children:[n.jsx("div",{className:`transit-caption ${l?"visible":""}`,children:l&&c&&`PROJECT ${s+1} OF ${i} — ${c.title} AHEAD`}),n.jsxs("div",{className:"scroll-hint",children:[n.jsx("span",{children:o==="orbit"?"SCROLL TO DESCEND":"SCROLL TO CONTINUE"}),n.jsx("div",{className:"scroll-hint__arrow"})]})]})]})}function on({scrollProgress:e,phase:s,phaseProgress:o,projectIndex:i,warpActive:r,finalMap:t,showSpacecraft:l,onReadMore:c,isMobile:u,mouseNDC:p}){var m;const h=a.useMemo(()=>T.map((d,b)=>{const x=b>0?T[b-1].orbPosition:null;return X(d,x)}),[]);return n.jsxs(n.Fragment,{children:[n.jsx(xe,{makeDefault:!0,position:[0,2,50],fov:55}),n.jsx(Ke,{scrollProgress:e,currentProjectIndex:i,currentPhase:s}),n.jsx(Ye,{warpStrength:r?1:0,isMobile:u}),n.jsx("fog",{attach:"fog",color:"#02040A",near:40,far:120}),n.jsx("ambientLight",{intensity:.08}),n.jsx("pointLight",{position:[0,30,-20],intensity:.5,color:"#3366ff"}),T.map((d,b)=>n.jsx(Ie,{project:d,phase:b===i?s:"idle",phaseProgress:b===i?o:0,mouseNDC:p,isActive:b===i,isMobile:u},d.id)),n.jsx(We,{project:T[i],phase:s,phaseProgress:o,onReadMore:c}),t&&h.map((d,b)=>n.jsx(Qe,{path:d,visible:t,opacity:.5},b)),n.jsx(Xe,{visible:t||e<.05,position:t?new C(12,4,-15):(m=T[i])==null?void 0:m.orbPosition.clone().add(new C(6,2,8))}),!u&&n.jsx(Ze,{phase:s,phaseProgress:o})]})}function un(){const e=a.useRef(null),s=de(D=>D.performanceLow),[o,i]=a.useState(0),[r,t]=a.useState("approach"),[l,c]=a.useState(0),[u,p]=a.useState(0),[h,m]=a.useState(0),[d,b]=a.useState(!1),[x,w]=a.useState(!1),[v,g]=a.useState(!1),[f,y]=a.useState(!1),[R,k]=a.useState(null),[Z,J]=a.useState({x:0,y:0});Re({phase:r,projectIndex:u,phaseProgress:l,zoneProgress:h}),a.useEffect(()=>{const D=I=>{J({x:I.clientX/window.innerWidth*2-1,y:-(I.clientY/window.innerHeight*2-1)})};return window.addEventListener("mousemove",D,{passive:!0}),()=>window.removeEventListener("mousemove",D)},[]),a.useEffect(()=>{if(!e.current)return;const D=me.create({trigger:e.current,start:"top top",end:"bottom bottom",scrub:1.5,onEnter:()=>b(!0),onUpdate:I=>{const F=I.progress;i(F);const{projectIndex:te,phase:L,phaseProgress:oe,zoneProgress:re}=Te(F,T.length);p(te),t(L),c(oe),m(re),y(L==="escape"),g(F>.97)}});return()=>D.kill()},[]);const ee=a.useCallback(()=>{k(T[u])},[u]),ne=a.useCallback(()=>{k(null)},[]);return n.jsx("div",{ref:e,className:"cartography-scroll-space",children:n.jsxs("div",{className:"cartography-sticky",children:[n.jsx(he,{gl:{antialias:!0,alpha:!1,powerPreference:"high-performance"},dpr:s?[1,1]:[1,1.5],style:{position:"absolute",inset:0,background:"#02040A"},children:n.jsx(on,{scrollProgress:o,phase:r,phaseProgress:l,projectIndex:u,warpActive:f,finalMap:v,showSpacecraft:!0,onReadMore:ee,isMobile:s,mouseNDC:Z})}),d&&!x&&n.jsx(nn,{visible:d,onComplete:()=>w(!0)}),n.jsx(Je,{project:T[u],visible:r==="orbit",phaseProgress:l}),n.jsx(en,{project:T[u],phase:r,phaseProgress:l}),v&&n.jsx("div",{className:"final-map-text",children:"EVERY WORLD BEGAN AS AN EMPTY CANVAS."}),n.jsx(tn,{scrollProgress:o,projectIndex:u,phase:r,totalProjects:T.length}),R&&n.jsx(qe,{project:R,onClose:ne})]})})}export{un as default};
