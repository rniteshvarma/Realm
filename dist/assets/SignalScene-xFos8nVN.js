import{M as Z,W as Q,c as W,H as ee,b,X as N,Y as te,Z as re,f as V,_ as ne,$ as U,V as A,B as Y,n as I,r as u,a as P,j as e,D as _,u as $,S as oe,J as F,C as se,a0 as ae,d as ie,e as ce,g as O,a1 as z}from"./index-CIeHNeq0.js";import{T as D}from"./Text-DWZWlkur.js";import{P as le}from"./PerspectiveCamera-DE9pibGa.js";import"./Fbo-Bw4Tt_vM.js";class B extends Z{constructor(a,r={}){super(a),this.isReflector=!0,this.type="Reflector",this.camera=new Q;const s=this,t=r.color!==void 0?new W(r.color):new W(8355711),l=r.textureWidth||512,f=r.textureHeight||512,i=r.clipBias||0,n=r.shader||B.ReflectorShader,d=r.multisample!==void 0?r.multisample:4,c=new ee,x=new b,h=new b,m=new b,M=new N,y=new b(0,0,-1),p=new U,v=new b,S=new b,g=new U,w=new N,j=this.camera,T=new te(l,f,{samples:d,type:re}),E=new V({name:n.name!==void 0?n.name:"unspecified",uniforms:ne.clone(n.uniforms),fragmentShader:n.fragmentShader,vertexShader:n.vertexShader});E.uniforms.tDiffuse.value=T.texture,E.uniforms.color.value=t,E.uniforms.textureMatrix.value=w,this.material=E,this.onBeforeRender=function(R,K,H){if(h.setFromMatrixPosition(s.matrixWorld),m.setFromMatrixPosition(H.matrixWorld),M.extractRotation(s.matrixWorld),x.set(0,0,1),x.applyMatrix4(M),v.subVectors(h,m),v.dot(x)>0)return;v.reflect(x).negate(),v.add(h),M.extractRotation(H.matrixWorld),y.set(0,0,-1),y.applyMatrix4(M),y.add(m),S.subVectors(h,y),S.reflect(x).negate(),S.add(h),j.position.copy(v),j.up.set(0,1,0),j.up.applyMatrix4(M),j.up.reflect(x),j.lookAt(S),j.far=H.far,j.updateMatrixWorld(),j.projectionMatrix.copy(H.projectionMatrix),w.set(.5,0,0,.5,0,.5,0,.5,0,0,.5,.5,0,0,0,1),w.multiply(j.projectionMatrix),w.multiply(j.matrixWorldInverse),w.multiply(s.matrixWorld),c.setFromNormalAndCoplanarPoint(x,h),c.applyMatrix4(j.matrixWorldInverse),p.set(c.normal.x,c.normal.y,c.normal.z,c.constant);const C=j.projectionMatrix;g.x=(Math.sign(p.x)+C.elements[8])/C.elements[0],g.y=(Math.sign(p.y)+C.elements[9])/C.elements[5],g.z=-1,g.w=(1+C.elements[10])/C.elements[14],p.multiplyScalar(2/p.dot(g)),C.elements[2]=p.x,C.elements[6]=p.y,C.elements[10]=p.z+1-i,C.elements[14]=p.w,s.visible=!1;const X=R.getRenderTarget(),q=R.xr.enabled,J=R.shadowMap.autoUpdate;R.xr.enabled=!1,R.shadowMap.autoUpdate=!1,R.setRenderTarget(T),R.state.buffers.depth.setMask(!0),R.autoClear===!1&&R.clear(),R.render(K,j),R.xr.enabled=q,R.shadowMap.autoUpdate=J,R.setRenderTarget(X);const k=H.viewport;k!==void 0&&R.state.viewport(k),s.visible=!0},this.getRenderTarget=function(){return T},this.dispose=function(){T.dispose(),s.material.dispose()}}}B.ReflectorShader={name:"ReflectorShader",uniforms:{color:{value:null},tDiffuse:{value:null},textureMatrix:{value:null}},vertexShader:`
