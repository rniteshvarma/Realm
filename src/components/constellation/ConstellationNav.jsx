...
//  Fixed 120×120 canvas, bottom-right.
//  9 organic star positions representing each Realm.
// ═══════════════════════════════════════════════════════

// Hand-crafted organic constellation positions (like Orion's Belt)
// Each: [x, y, mass (node size), name]
const STARS = [
  { x: 60, y: 15,  mass: 5,   name: 'GATE',         realm: 0 },
  { x: 25, y: 35,  mass: 4,   name: 'MANIFESTO',    realm: 1 },
  { x: 95, y: 42,  mass: 4.5, name: 'ARSENAL',      realm: 2 },
  { x: 40, y: 65,  mass: 3.5, name: 'ARCHIVE',      realm: 3 },
  { x: 80, y: 70,  mass: 4,   name: 'SIGNAL',       realm: 4 },
  { x: 20, y: 88,  mass: 5,   name: 'ECHO CHAMBER', realm: 5 },
  { x: 55, y: 95,  mass: 4.5, name: 'FREQUENCY',    realm: 6 },
  { x: 90, y: 100, mass: 4,   name: 'MYTH ENGINE',  realm: 7 },
  { x: 110, y: 80, mass: 6,   name: 'TRANSCENDENCE',realm: 8 },
]
...
export default function ConstellationNav() {
  ...
  // Detect current realm from scroll position
  useEffect(() => {
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          const realmIdx = parseInt(entry.target.dataset.realm, 10)
          if (!isNaN(realmIdx)) {
            useStore.getState().setRealm(realmIdx)
            useStore.getState().markRealmVisited(realmIdx)
          }
        }
      })
    }, { threshold: 0.4 })

    const sections = document.querySelectorAll('[data-realm]')
  ...
  const draw = useCallback(() => {
    ...
    stars.forEach(star => {
      ...
      const isSecret   = star.realm === 9  // Realm IX — hidden until all eggs found

      if (isSecret && !allEggsFound) return
      ...
      // Secret 9th star burns brightest
      if (isSecret && allEggsFound) {
        ctx.beginPath()
        ctx.arc(star.rx, star.ry, r + 2, 0, Math.PI * 2)
        ctx.fillStyle = '#fff'
        ctx.shadowBlur = 30
        ctx.shadowColor = '#C9A84C'
        ctx.fill()
  ...
  }, [getTransformedStars, allEggsFound])
